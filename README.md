...
Browser DevTools "device emulation" is a lie — it can not reproduce real DPR, real touch mechanics, hardware rendering quirks, or the actual viewport a user sees. So you either skip real-device testing (and ship bugs) or you fight through one of these:
...
1. **Proxy** — devmirror starts a transparent HTTP proxy in front of your dev server. All requests are forwarded; CORS and `Host` headers are rewritten automatically. HMR WebSocket connections pass through untouched.

2. **Script injection** — for every HTML response, devmirror injects a small bridge script (`/__bridge.js`) before `</body>`. The script is vanilla JS, under 3 KB, zero dependencies.

3. **WebSocket bridge** — the bridge script on the phone connects back to devmirror over a WebSocket. It forwards console output, touch events, DOM snapshots, and device info in real time.
...
> **`--tunnel` is planned for v1.0** — for when your phone and laptop are on different networks (coffee shops, client offices). Until then both devices must be on the same WiFi.
...
devmirror starts automatically when you run `vite dev`. It reads the port Vite is already listening on — no `-p` needed.
...
devmirror is a Node.js tool built entirely from standard npm packages — no compiled binaries, no native modules, no build step required to run the source.
...
| `--tunnel` flag (ngrok — works across networks) | v1.0 |
