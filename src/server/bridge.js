...
const PHONE_TO_DEVTOOLS_TYPES = new Set(['dimensions', 'console', 'touch', 'dom']);

// Message types that flow from DevTools → bridge → phone.
const DEVTOOLS_TO_PHONE_TYPES = new Set(['request_dom', 'highlight', 'reload']);
 ...
 * @returns {void}
 */
function handleConnection(socket, phoneClients, devtoolsClients) {
  ...
  socket.once('message', (data) => {
    ...
    if (role === 'phone') {
      ...
      // Notify all open DevTools panels that a phone has joined.
      broadcast(devtoolsClients, { type: 'phone_connected' });
    } else {
...
export function startBridge({ bridgePort }) {
  return new Promise((resolve, reject) => {
    ...
    server.on('connection', (socket) => {
      handleConnection(socket, phoneClients, devtoolsClients);
    });
